...
# ############################################################################
# [2018-12-04 00:51:03.027000] THIS FILE IS AUTOGENERATED - DO NOT EDIT!
# ############################################################################
...
class Proxy(ProxyBase):
    ...
    def analog_reference(self, type):
        command = np.dtype('uint16').type(self._CMD_ANALOG_REFERENCE)
        ARG_STRUCT_SIZE = 1
        array_data = b''
        payload_size = ARG_STRUCT_SIZE + len(array_data)
        struct_data = np.array([(type, )],
                               dtype=[('type', 'uint8'), ])
        payload_data = struct_data.tostring() + array_data
    ...
    def array_length(self, array):
        ...
        array_data = b''.join([array.tostring(), ])
        payload_size = ARG_STRUCT_SIZE + len(array_data)
    ...
    def str_echo(self, msg):
        ...
        array_info = pd.DataFrame([msg.shape[0], ],
                                  ...
                                  columns=['length'])
        array_info['start'] = array_info.length.cumsum() - array_info.length
        array_data = b''.join([msg.tostring(), ])
        payload_size = ARG_STRUCT_SIZE + len(array_data)
    ...
    def update_eeprom_block(self, address, data):
        ...
        array_info = pd.DataFrame([data.shape[0], ],
                                  ...
                                  columns=['length'])
        array_info['start'] = array_info.length.cumsum() - array_info.length
        array_data = b''.join([data.tostring(), ])
        payload_size = ARG_STRUCT_SIZE + len(array_data)
    ...
    def update_config(self, serialized):
        ...
        array_info = pd.DataFrame([serialized.shape[0], ],
                                  ...
                                  columns=['length'])
        array_info['start'] = array_info.length.cumsum() - array_info.length
        array_data = b''.join([serialized.tostring(), ])
        payload_size = ARG_STRUCT_SIZE + len(array_data)
