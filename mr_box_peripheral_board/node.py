...
# ############################################################################
# [2024-11-19 15:24:19.413173] THIS FILE IS AUTOGENERATED - DO NOT EDIT!
# ############################################################################
import pandas as pd
import numpy as np
...
class Proxy(ProxyBase):
    ...
    _CMD_ANALOG_REFERENCE = 0x10
    _CMD_ARRAY_LENGTH = 0x11
    _CMD_ECHO_ARRAY = 0x12
    _CMD_STR_ECHO = 0x13
    ...
    def base_node_software_version(self):
        ...
        result = np.frombuffer(response.data(), dtype='uint8')
    ...
    def microseconds(self):
        ...
        result = np.frombuffer(response.data(), dtype='uint32')
    ...
    def analog_read(self, pin):
        ...
        result = np.frombuffer(response.data(), dtype='uint16')
    ...
    def analog_reference(self, type_):
        command = np.dtype('uint16').type(self._CMD_ANALOG_REFERENCE)
        ARG_STRUCT_SIZE = 1
        array_data = b''
        payload_size = ARG_STRUCT_SIZE + len(array_data)
        struct_data = np.array([(type_, )],
                               dtype=[('type_', 'uint8'), ])
        payload_data = struct_data.tobytes() + array_data
    ...
    def array_length(self, array):
        ...
        array_data = b''.join([array.tobytes(), ])
        payload_size = ARG_STRUCT_SIZE + len(array_data)
    ...
    def max_serial_payload_size(self):
        ...
        result = np.frombuffer(response.data(), dtype='int32')
    ...
    def update_eeprom_block(self, address, data):
        ...
        array_info = pd.DataFrame([data.shape[0], ],
                                  ...
                                  columns=['length'])
        array_info['start'] = array_info.length.cumsum() - array_info.length
        array_data = b''.join([data.tobytes(), ])
        payload_size = ARG_STRUCT_SIZE + len(array_data)
    ...
    def update_config(self, serialized):
        ...
        if isinstance(serialized, str):
            ...
            serialized = list(bytes(serialized))
        # Argument is an array, so cast to appropriate array type.
        ...
        array_info = pd.DataFrame([serialized.shape[0], ],
                                  ...
                                  columns=['length'])
        array_info['start'] = array_info.length.cumsum() - array_info.length
        array_data = b''.join([serialized.tobytes(), ])
        payload_size = ARG_STRUCT_SIZE + len(array_data)
    ...
    def pmt_reference_voltage(self):
        ...
        result = np.frombuffer(response.data(), dtype='float32')
    ...
    def set_id(self, id_):
        ...
        if isinstance(id_, str):
            ...
            id_ = list(bytes(id_))
        # Argument is an array, so cast to appropriate array type.
        id_ = np.ascontiguousarray(id_, dtype='uint8')
        array_info = pd.DataFrame([id_.shape[0], ],
                                  index=['id_', ],
                                  columns=['length'])
        array_info['start'] = array_info.length.cumsum() - array_info.length
        array_data = b''.join([id_.tobytes(), ])
        payload_size = ARG_STRUCT_SIZE + len(array_data)
        struct_data = np.array([(array_info.length['id_'], ARG_STRUCT_SIZE + array_info.start['id_'], )],
                               dtype=[('id__length', 'uint32'), ('id__data', 'uint16'), ])
        payload_data = struct_data.tobytes() + array_data
